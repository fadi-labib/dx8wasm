...
Phased **independent re-derivation** into a decoupled, Linux-CI'd SDK. The working reference (the Lolendor/Generals-WebAssembly web port) and GeneralsX/EA are read-only sources we study, not extract from (see `docs/LICENSING.md`). Each phase ends with a runnable/verifiable artifact.

**What is in flight right now:** not all phases below are active. The live work is telemetry and measured-gap verification against the Generals integration — the current evidence base is the "Measured against a real target" section of [`CONFORMANCE.md`](CONFORMANCE.md#measured-against-a-real-target-not-empirically-probed) and [`measured-gap.json`](measured-gap.json). Phases 5 and 6 are parked (see below); everything else is done — the Phase 3 measured-gap tail that this line used to point at was closed on 2026-08-01 (see below).
...
- Build `runtime/platform/` (SDL3 window/input) as a thin new layer. (Audio stayed game-side; no OpenAL layer was built.)
...
- ✅ **Measured-gap tail closed (2026-08-01)** — every token in
  ...
  [`RESULTS-2026-08-01-close-the-docs-items.md`](RESULTS-2026-08-01-close-the-docs-items.md) (the plan itself was a
  machine-specific recipe and was retired from the tree on 2026-09-03; its outcome and lessons are in that results doc).
...
- ✅ **Capstone done:** [`CONFORMANCE.md`](CONFORMANCE.md) — a conformance matrix generated by `scripts/conformance.mjs`. The `conformance` probe program exercises each D3D8 token against a live device and reads the coverage counters to classify handled vs fallback (empirical, can't drift); the feature table is curated and paired with each verifying smoke. Current counts live in the generated table itself — see [`CONFORMANCE.md`](CONFORMANCE.md), never copy its numbers into prose here (a hardcoded copy of a generated table is a permanent liability, see CDI-4 in the lessons library).
...
## Phase 4 — CI & tests
- ✅ **CI harness done:** `scripts/check.sh` (mechanical guardrails: SPDX headers, commit-authorship, no saturating 32-bit casts of `emscripten_get_now()`) + `scripts/ci.sh` (guardrails + pinned-toolchain check + the full test suite — every smoke listed in `web-runtime/test/phase2.gpu.test.mjs`, spanning the d3d8webgl/compatlib/telemetry surface — every `CMakeLists.txt` executable target except `conformance`, `minigame`, and `spin_demo`) + `.github/workflows/ci.yml`, which just invokes `ci.sh`. Runs locally and on every push: the GitHub Actions workflow (emsdk 6.0.2 via `setup-emsdk`, Playwright
Chromium with SwiftShader WebGL2) has been green on `main` since 2026-08-07, so CI-on-push is a gate
that passes, not an aspiration.
...
**Phase 4 complete.** Every non-parked phase is now closed; the open list is the two parked
phases (Phase 5, Phase 6) plus the items already named above under "Game-integration
foundation" — compatlib's grow-on-demand tiers, the Phase A bring-up harness/CMake seam, and
the game-side items (non-blocking main loop conversion, asset wiring, audio). None of those
is a phase in its own right; they are integration follow-ups this roadmap tracks in place
...
card was never the constraint. The measurement, and the seven hypotheses it eliminated, are written
up in the Generals integration repo's ANGLE-backend results (2026-08-06).
...
The independently written `d3d8.h` is now the **complete standard D3D8 interface** in canonical
...
(tracked in the integration repo's plan). Next Phase A: bring-up harness + CMake seam.
...
- **Rendering lessons from the 2026-08-28 Generals integration** (`AB-33..35`) live in the
  integration repo's lessons file for that date (`generals-dx8wasm`, `docs/LESSONS-2026-08-28.md`),
