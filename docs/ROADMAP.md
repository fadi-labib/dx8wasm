...
- Continue hardening `graphics-ff` against **DXVK `d3d9_fixed_function.cpp`** (2,661 lines, zlib/permissive — local copy at `Generals-Mac-iOS-iPad/references/fadi-labib-dxvk/`) for specular, EXP/EXP2 fog, more `D3DTOP_*` combiners, vertex-blend. Cross-check behavior against Wine `wined3d` but **never paste** (LGPL).
