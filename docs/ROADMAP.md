...
- ⏳ **2.5 next:** coverage/fallback layer — every unhandled `D3DRS_*`/`D3DTSS_*`/`D3DFMT_*` logs loudly, counts, and falls back via the `contract.h` hooks (`dx8wasm_get_coverage`, `dx8wasm_set_unhandled_callback`).
